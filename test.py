...
parser.add_argument('--model_path', type=str, default="./save_models/", help='model_path')
parser.add_argument('--file_type', type=str, default="csv", help="dataset file type, csv or h5")
...
def main():
    ...
    # load model 
    ckpt_file = os.path.join(args.model_path, 'best_model.pth.tar')
...
def test(val_loader, model, criterion, mode):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        print("output is 0:", torch.all(output == 0))
        pred[i, :, :, :] = output.cpu().data.float() #output.data.float()
